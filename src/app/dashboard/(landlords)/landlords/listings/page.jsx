  ...
  const fetchData = async () => {
    try {
      const res = await axios.post(
        "https://kuda-creditclan-api.herokuapp.com/agents/getAgentProperty",
  ...
  return (
    <>
      ...
      <div className="flex justify-between">
        <div>
          <div className="text-4xl font-bold">Hey, {data?.user?.name} 👋🏿</div>
        </div>
        <div>
          <Button
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-4 md:gap-6 xl:grid-cols-4 2xl:gap-7.5">
          {userData?.map((user, i) => (
            <div className=" bg-white shadow-lg rounded-xl p-1" key={i}>
              <div
                className="bg-cover h-52 p-2"
                style={{
                  backgroundImage: `url(${user?.image[0]})`,
                  borderRadius: "10px",
                }}
              >
              ...
              <div className="p-3 space-y-2">
                <p className="font-bold text-xl flex justify-between">
                  ...
                  <div className="bg-red-100 p-2 rounded-full">
                    <IconHeartFilled size="15" className="text-red-500" />
                  </div>
                </p>
                <p className="font-bold"> {user?.name}</p>
                <p className="inline-flex text-sm text-gray-400">
                  {" "}
                  <IconMapPinFilled
                    color="red"
                    size={15}
