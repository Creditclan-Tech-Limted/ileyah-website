...
const CheckOffers = ({ className, isOpen, onClose }) => {
  ...
  const checkOffers = async () => {
    try {
      ...
      const res = await axios.post(
        "https://mobile.creditclan.com/api/v3/schoolportal/offers",
        {
          request_id: data?.user?.lend_loan_id,
          token: data?.user?.lend_token,
        },
        {
          headers: {
            "x-api-key":
      ...
      updateData({
        ...
        user: {
          ...data?.user,
          credit_score: res?.data?.payload?.repayment_amount?.toFixed(2),
        },
      });
      setLoading(false);
      setViews("view-result");
    ...
    try {
      ...
      let body = {
        profile: {
          ...
          email: data?.user?.email
            ? data?.user?.email
            : `${data?.user?.phone + new Date().getTime()}@gmail.com`,
      ...
      if (
        ...
        res?.data?.message.includes(
          "You seem to already have an account with us. Please login instead"
        )
      ) {
  ...
  const analyzeBS = async (res, resi, values) => {
    try {
      ...
      const response = await axios.post(
        "https://wasapnodeserver.herokuapp.com/analyze",
        {
          password: "",
          file_url,
          name: data?.user?.name,
    ...
    <>
      <Drawer title="Credit Check" isOpen={isOpen} onClose={onClose}>
        ...
        {views === "view-result" && isOfferLoading === "show-offer-loading" && (
          <div>
            <h3 className="text-xl font-medium mb-8 px-1 border-b pb-6 mt-[50px]">
              Analysis Result
            </h3>
              ...
              <div className="flex justify-between">
                ...
                <div className="my-auto">
                  <p className="text-2xl">
                    {" "}
                    {offers?.repayment_amount?.toFixed(2)} / mo
                  </p>
                </div>
              </div>
            </div>
          ...
          {views === "inspections" && (
            <>
              <div className="bg-red-600 p-10 rounded-xl text-white">
                ...
                <div className="mt-5">
                  <div className="space-y-4">
                    Monitoring your credit score helps you stay informed about
                    your creditworthiness. It provides insights into your
                    financial health and whether you're in a good position to
                    borrow money or make significant financial decisions.
                  </div>
                </div>
              </div>
              <h3 className="text-xl font-medium mb-8 px-1 border-b pt-10 pb-6">
                Please fill in the informtion below
              </h3>
              <form onSubmit={handleSubmit(onSubmit)}>
                ...
                <Button className="mt-10" type="submit" loading={loading}>
                  Submit
                </Button>
              </form>
            </>
