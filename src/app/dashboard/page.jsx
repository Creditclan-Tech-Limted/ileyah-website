...
const Page = ({ className }) => {
  ...
  const { mutateAsync: checkUser, isLoading: isCheckUserLoading } =
    useCheckRentRequestMutation();
  const { mutateAsync: getInspections, isLoading: isGetInspectionsLoading } =
    useGetInspectionDetails();
  ...
  const getRecoveryInfo = async (creditclan_request_id) => {
    try {
      const res = await axios.post(
        ...
        { creditclan_request_id: creditclan_request_id },
        {
          headers: {
            "x-api-key":
      ...
      if (res?.data?.data) {
        ...
        if (!hasFullyPaid) {
          overdueBalances = schedule?.filter((i) => {
            return (
              isBefore(new Date(i.repayment_date), new Date()) &&
              +i.how_much_remaining > 0
            );
          });
          if (overdueBalances.length > 0) {
            nextPayment = overdueBalances.reduce(
              (acc, cur) => acc + +cur?.how_much_remaining,
              0
            );
            nextPaymentDate = overdueBalances[0]?.repayment_date;
            const next = schedule.find(
              (s) =>
                isAfter(new Date(s.repayment_date), new Date()) &&
                +s.how_much_remaining > 0
            );
            nextTillEnd = schedule.filter(
              (s) =>
                isAfter(new Date(s.repayment_date), new Date()) &&
                +s.how_much_remaining > 0
            );
            nextTillEndTotal = nextTillEnd.reduce(
              (acc, cur) => acc + +cur?.how_much_remaining,
              0
            );
            nextMonth = next;
  ...
  const getPorperties = async () => {
    try {
      const res = await axios.get(
        "https://lendnode.creditclan.com/kuda/get_properties"
      );
  ...
  const getUpfrontStatus = async (creditclan_request_id) => {
    try {
      const res = await axios.post(
        "https://wema.creditclan.com/api/v3/wema/getUpfrontStatus",
    ...
    <>
              ...
              <div>
                {isGetInspectionsLoading ? (
                  ...
                  <>
                                    ...
                                    <div className="grid  border border-gray-300 rounded-xl overflow-hidden">
                                      <div className="flex p-5  items-center ">
                                        ...
                                        <div className="px-5 overflow-hidden">
                                          ...
                                          <div className="">
                                            <p>
                                              Next repayment due by{" "}
                                              {recoveryFilter?.nextPaymentDate}{" "}
                                            </p>
                                          </div>
                                        </div>
                                      </div>
                    ...
                    <h3 className="text-xl font-medium mb-8 px-1 my-20">
                      Market Place
                    </h3>
      ...
      >
        <ViewPropertyDetails
          ...
          onClose={() => setOpenViewProperty(false)}
          upfront={upfront}
          isGetLoanDetailsLoading={isGetLoanDetailsLoading}
        />
      </Drawer>
      <InspectionDetails
        isOpen={openViewInspections}
        onClose={() => setopenViewInspections(false)}
        inspection={current}
      />
      <RenewRentDashboard
      ...
      />
      <CheckOffers
        isOpen={openCheckOffers}
        onClose={() => setOpenCheckOffers(false)}
      />
      <FoundHouseDashboard
        isOpenDrawer={isOpenFoundHouse}
        onClose={() => setIsOpenFoundHouse(false)}
      />
      {data?.user?.want_this && <WantThis />}
      <MakePayment
        isOpen={openMakePayment}
