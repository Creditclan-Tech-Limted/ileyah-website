...
import { IconChevronDown, IconCopy, IconHomeSearch, IconLogout, IconPlus } from '@tabler/icons-react'
...
const Page = ({ className }) => {
  ...
  return (
    <>
          ...
          !loading && foundHouse?.map((f, i) => (
            <div className='bg-white shadow border flex rounded-2xl p-3' key={i}>
              ...
              <div className='my-auto ml-auto space-x-3'>
                <Button size='sm' leftIcon={<IconPlus />} onClick={() => {
                  setcurrent(f)
      ...
      <AddNewProperty isOpen={openComments} onClose={() => setOpenComments(false)} current={current} />
    </>
  )
}
